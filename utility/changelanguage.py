...
from utility.common import start_activity
import re
def changeLanguage(self, languageTo):
	...
	view = None
	self(scrollable=True).scroll.vert.toEnd()
	#find the text "Language & input" with different language
	for k, v in LANGUAGE_SETTINGS.iteritems():
		if self(text=v).exists:
			currentLanguage = k
			break
	if currentLanguage is None:
		raise ViewNotFoundException("text", "'Language & input' (any language)", "ROOT")
	#click "Language & input" item
	self(text=v).click()
	#click "Language " item
	self(text=PHONE_LANGUAGE[currentLanguage]).click()
	# try to find the language
	self(resourceId="android:id/list").scroll.vert.to(text=u"English (United States)")
	# found it, and click it.
	self(text=u"English (United States)").click()
